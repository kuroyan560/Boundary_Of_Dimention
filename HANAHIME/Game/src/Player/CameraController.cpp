...
void CameraController::Update(KuroEngine::Vec3<float> arg_scopeMove, KuroEngine::Transform arg_playerTransform, float arg_cameraZ, const std::weak_ptr<Stage> arg_nowStage)
{
	...
	//トランスフォームを適用
	m_oldPlayerTransform = m_playerTransform;
	m_playerTransform = arg_playerTransform;

	//プレイヤーの移動をカメラに適用
	m_baseTransform.SetPos(m_baseTransform.GetPos() + (m_playerTransform.GetPos() - m_oldPlayerTransform.GetPos()));




	//一旦カメラのトランスフォームをアタッチ。
	m_attachedCam.lock()->GetTransform() = m_baseTransform;
